...
class Multiport(object):
    def __init__(self, raw):
        ...
        for i in range(len(fields)):
            if fields[i] == "--source-ports" or fields[i] == "--sports":
                if fields[i-1] == "!":
                    self.invert_sports = True
                if "," in fields[i+1]:
                    for p in fields[i+1].split(","):
                        self.sports.append(int(p))
                elif ":" in fields[i+1]:
            ...
            if fields[i] == "--destination-ports" or fields[i] == "--dports":
                if fields[i-1] == "!":
                    self.invert_dports = True
                if "," in fields[i+1]:
                    for p in fields[i+1].split(","):
                        self.dports.append(int(p))
                elif ":" in fields[i+1]:
                    start = int(fields[i+1].split(":")[0])
                    end = int(fields[i+1].split(":")[1])
                    for p in range(start, end+1):
